...
业务在图片生成基础上新增**完全独立的视频生成能力**——独立接口、独立编排、独立 generator、独立配置，仅复用钉钉客户端与基础设施（日志、中间件、异常处理、依赖注入）。

### 9.1 与图片生成的边界

| 维度 | 图片生成 | 视频生成 |
|------|---------|---------|
| 入口接口 | `POST /api/v1/generate` | `POST /api/v1/video/generate` |
| 表格配置 | `[[dingtalk.tables]]`（含 clothing/poster 等） | `[[dingtalk.video_tables]]`（含 zhuozhi-video/huapu-video/ahmi-video） |
| 引擎 | `AIGenerator`（google/openai SDK） | `VideoGenerator`（httpx 直连三家 API） |
| 编排服务 | `GenerationService`（7 步） | `VideoGenerationService`（9 步） |
| 并发控制 | `settings.server.max_concurrency` (5) | `settings.server.video_max_concurrency` (3) |
| 模型字段 | 钉钉表格"生图模型"（下拉选项，服务侧有"显示名 → API model"映射） | 钉钉表格"视频模型"（下拉选项，**原值透传**，零翻译） |
| 调用模式 | 同步生成（一次返回字节） | 异步任务（提交 → 轮询 → 取结果 URL） |
| 上传附件 MIME | `image/png` | `video/mp4` |
| 耗时 | 几秒 | 30s–5min |

### 9.2 9 步编排（与图片 7 步骨架对称）

```
[1] 获取视频表格配置（按 table_key 命中 video_tables）
[2] 获取钉钉记录（取 提示词 / 视频模型 / 首帧图）
[3] 校验：提示词 / 视频模型 / 首帧图 三必填
[4] 下载首帧图字节
[5] base64 编码首帧图（VideoGenerator 内部）
[6] 提交视频任务到第三方 API → task_id（VideoGenerator 内部）
[7] 轮询任务状态直至 succeed/Success/SUCCEEDED（VideoGenerator 内部）
[8] 下载 mp4 字节，上传到钉钉云空间（media_type=video/mp4）
[9] 回写表格：生成视频附件 + 生成结果 + 生成时间
```

步骤 5–7 在 `VideoGenerator.generate()` 内部完成（与图片生成步骤 5"调用 AI"对称），步骤 1/2/3/4/8/9 在 `VideoGenerationService.process()` 中执行。
...
客户端 ──POST──▶ /kling/v1/videos/image2video ──▶ task_id
                  ...
                  │ /alibailian/.../video-synthesis
                  ▼
        GET /tasks/{task_id}  ──▶ 状态（submitted/processing/succeed/failed）
                  │
                  ▼
     状态=succeed ──▶ data.task_result.videos[0].url ──▶ 下载 mp4
```

统一轮询策略（`[ai.video.poll]` 配置）：
- 初始等待 10s
- 此后每 5s 轮询一次
- 总超时 10 分钟（`max_total=600`）

### 9.4 Provider 路由（model_name 前缀匹配）

服务侧**不维护"显示名 → API model_name"映射**。钉钉表格"视频模型"字段直接填第三方 API 原值（如 `kling-v2-5-turbo` / `MiniMax-Hailuo-2.3` / `happyhorse-1.0-i2v`），服务侧仅按前缀判断走哪个 provider：

```python
def _resolve_provider(model_name: str) -> str:
    if model_name.startswith("kling"):           return "kling"
    if model_name.startswith(("MiniMax-Hailuo", "MiniMax", "Hailuo")): return "hailuo"
    if model_name.startswith(("happyhorse", "wanx", "wan")):           return "wanxiang"
```

新增厂商/模型无需改 toml 配置，只在 `_resolve_provider` 加前缀 + 在 `VideoGenerator` 加 `_submit_x` / `_poll_x` 方法。

### 9.5 长任务并发模型

- `Semaphore(3)`（`settings.server.video_max_concurrency`）：限制最大并发视频任务数
- 与图片 `Semaphore(5)` 完全独立，系统总并发上限 = 8
- 轮询期间 `asyncio.sleep` 占槽位但不占 CPU，符合预期（限制对第三方 API 的并发压力）
- 失败回写复用 `_update_failure` 模式

...
| 模型 | SDK | 中转站 base_url |
|------|-----|-----------------|
| Nano Banana Pro | Google genai SDK | 可配置 |
| Nano Banana 2 | Google genai SDK | 可配置 |
| GPT Image 2 | OpenAI SDK | 可配置 |
...
**模型调用细节**：
- **Google genai SDK**（NanoBanana）：`contents=[prompt, image]`，image 可以是 PIL Image 或 bytes；返回的 `inline_data` 格式不固定（可能是 PNG/JPEG）
- **OpenAI SDK**（GPT-Image）：`image=[file_object_1, ...]`，传文件句柄或 bytes；返回 `result.data[0].b64_json` → `base64.b64decode()` 转 bytes；输入输出均为 PNG
...
| 钉钉表格值 | provider | 真实 model_name | 中转站 endpoint |
|------------|----------|-----------------|-----------------|
| `Nano Banana Pro` | google | `gemini-3-pro-image-preview` | `/v1beta/models/...:generateContent` |
| `Nano Banana 2` | google | `gemini-3.1-flash-image-preview` | `/v1beta/models/...:generateContent` |
| `GPT Image 2` | openai | `gpt-image-2` | `/v1/images/edits` |
...
- **AI API Key**：NanoBanana 系列（Pro / 2）共用一个 key，GPT Image 2 独立一个 key，均通过中转站调用

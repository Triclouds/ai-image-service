...
3. assign_sousuo_index 序号分配（1-9）
4. 端到端 _process_batch_sousuo 流程（9 张全成功 + 自定义命名）
...
OUTPUT_ORDER = ["细节图", "场景图", "白底图"]  # 序号 1-3/4-6/7-9
COUNT_PER_SECTION = 3
...
def test_build_sousuo_prompts_output_order():
    """build_sousuo_prompts 按 output_order 输出 9 个 (prompt, 段名) 对。"""
    sections = parse_prompt_sections(PROMPT_TEXT, SECTION_TITLES)
    ...
    assert len(out) == 9
    # 段名顺序：细节图 × 3 → 场景图 × 3 → 白底图 × 3
    names = [tname for _, tname in out]
    assert names == ["细节图"] * 3 + ["场景图"] * 3 + ["白底图"] * 3
...
def test_assign_sousuo_index_1_to_9():
    """assign_sousuo_index 按 output_order 分配 1-9 序号。"""
    sections = parse_prompt_sections(PROMPT_TEXT, SECTION_TITLES)
...
async def test_sousuo_process_full_success(sousuo_settings, mock_dingtalk, mock_generator):
    """完整 9 张成功：文件名按 record_id_goodsId_shopCode_1~9.png 命名。"""
    # 模拟生图表记录
    ...
    mock_generator.generate_batch.return_value = [f"img{i}".encode() for i in range(1, 10)]
    ...
    expected_filenames = [
        f"recS_001_GOODS_001_13706801_{i}.png" for i in range(1, 10)
    ]
    assert upload_filenames == expected_filenames

    # 4. update_record 收到 9 个附件
    fields = mock_dingtalk.update_record.call_args[0][2]
    assert len(fields["生成结果"]) == 9
    assert fields["状态"] == "成功9/9"
...
async def test_sousuo_shop_code_extraction(
    ...
    for shop_raw, expected_code in test_cases:
        ...
        mock_generator.generate_batch.return_value = [b"i"] * 9
...
async def test_sousuo_process_partial_failure(
    ...
    """部分失败：9 张里 3 张 None → upload × 6，状态含成功计数。"""
    mock_dingtalk.get_record.return_value = {
        "id": "recS_P",
        "fields": {
            "商品ID": "GID",
    ...
    mock_dingtalk.download_file.return_value = b"ref"
    # 9 次 upload（实际只 6 次会被 await，但 side_effect 长度要 >= 9 才不抛 StopIteration）
    mock_dingtalk.upload_attachment.side_effect = [
        {"filename": f"f{i}.png", "size": 1, "type": "image/png", "url": f"/u{i}", "resourceId": f"r{i}"}
        for i in range(1, 10)
    ]
    # 9 个结果里 3 个 None
    results = [b"img1", None, b"img3", None, b"img5", b"img6", None, b"img8", b"img9"]
...
6、f"""
    ...
    fields = mock_dingtalk.update_record.call_args[0][2]
    assert "缺少段" in fields["状态"]
    assert "细节图" in fields["状态"]
